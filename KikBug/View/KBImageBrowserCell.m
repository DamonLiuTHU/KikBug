...
@interface KBImageBrowserCell ()<DNTapDetectingImageViewDelegate,UIScrollViewDelegate>
@property (strong,nonatomic) UIImage *image;
@end

@implementation KBImageBrowserCell
...
#pragma mark - set
- (void)bindModel:(NSString *)url
{
    MBRoundProgressView *progressView = [[MBRoundProgressView alloc] init];
    [self addSubview:progressView];
    [progressView autoSetDimensionsToSize:CGSizeMake(30, 30)];
    [progressView autoAlignAxisToSuperviewMarginAxis:ALAxisVertical];
    [progressView autoAlignAxisToSuperviewMarginAxis:ALAxisHorizontal];
...
- (UIScrollView *)zoomingScrollView
{
    if (nil == _zoomingScrollView) {
        _zoomingScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
