...
- (void)configConstrains
{
    ...
    [self.placeholderLabel autoPinEdgeToSuperviewMargin:ALEdgeTop];
    [self.placeholderLabel autoPinEdgeToSuperviewMargin:ALEdgeLeft];

    //    [self.descTextView autoPinEdgesToSuperviewEdgesWithInsets:UIEdgeInsetsZero excludingEdge:ALEdgeTop];
    //    [self.descTextView autoPinEdge:ALEdgeBottom toEdge:ALEdgeTop ofView:self.toolbar];
    [self.descTextView autoPinEdgeToSuperviewEdge:ALEdgeLeft];
    [self.descTextView autoPinEdgeToSuperviewEdge:ALEdgeRight];
    self.descTextConstraint = [self.descTextView autoPinEdgeToSuperviewEdge:ALEdgeBottom withInset:self.toolbar.height];
    [self.descTextView autoSetDimension:ALDimensionHeight toSize:100];
...
{
    self.descTextView = [UITextView new];
    self.descTextView.textColor = [UIColor lightGrayColor];
    self.descTextView.alpha = 0.5f;
    self.descTextView.showsVerticalScrollIndicator = YES;
    self.descTextView.indicatorStyle = UIScrollViewIndicatorStyleWhite;
    //    self.descTextView.textColor = THEME_COLOR;
...
- (void)keyboardWillShow:(NSNotification*)aNotification
{
    ...
    //    [self.descTextView autoPinEdgeToSuperviewEdge:ALEdgeBottom withInset:MAX(toolbarheight, height)];
    [self.descTextConstraint setConstant:-height];
...
- (void)keyboardWillHide:(NSNotification*)aNotification
{
    self.keyboardHeight = 0.0f;
    CGFloat toolbarheight = self.toolbar.height;
    [self.descTextConstraint setConstant:-toolbarheight];
}
@end
